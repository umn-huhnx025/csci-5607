...
// Shader sources
const GLchar* vertexSourcePhong =
    "#version 150 core\n"
    "in vec3 position;"
    //"in vec3 inColor;"
    "const vec3 inColor = vec3(0.f,0.7f,0.f);"
    "const vec3 inlightDir = normalize(vec3(1,1,1));"  // we should really pass
                                                       // this in
    "in vec3 inNormal;"
    "out vec3 Color;"
    "out vec3 normal;"
    ...
    "   pos = pos4.xyz/pos4.w;"  // Convert vec4 pos to vec3 ... typically
                                 // pos4.w and you don't need the divide
    ...
    "   normal = norm4.xyz;"  // Convert the vec4 to a vec3 ... we don't need to
                              // normalize if the above line is correct
    "   lightDir = (view * vec4(inlightDir,0)).xyz;"  // Transform light into to
                                                      // view space
    "}";
...
const GLchar* fragmentSourceBlinnPhong =
    "#version 150 core\n"
    "in vec3 Color;"
    "in vec3 normal;"
    "in vec3 pos;"
...
int main(int argc, char* argv[]) {
  const GLchar *fragmentSource, *vertexSource;

  if (argc < 2) {
    fragmentSource = fragmentSourcePhong;
    vertexSource = vertexSourcePhong;
  } else {
    switch (atoi(argv[2])) {
      case 1:
        fragmentSource = fragmentSourceBlinnPhong;
        vertexSource = vertexSourcePhong;
        break;
      case 2:
        fragmentSource = fragmentSourceGouraud;
        vertexSource = vertexSourceGouraud;
        break;
      default:
        fragmentSource = fragmentSourcePhong;
        vertexSource = vertexSourcePhong;
  ...
  glBindBuffer(GL_ARRAY_BUFFER, vbo[0]);  // Set the vbo as the active array
                                          ...
                                          // active at a time)
  glBufferData(GL_ARRAY_BUFFER, numLines * sizeof(float), modelData,
  ...
  // Maybe we need a second VAO, e.g., some of our models are stored in a second
  // format  GLuint vao2;  glGenVertexArrays(1, &vao2); //Create the VAO
  ...
  while (true) {
    if (SDL_PollEvent(&windowEvent)) {
      ...
      SDL_SetWindowFullscreen(
          window, fullscreen ? SDL_WINDOW_FULLSCREEN : 0);  // Toggle fullscreen
    ...
    model =
        glm::rotate(model, rot_x * 3.14f / 5.f, glm::vec3(0.0f, 0.0f, 1.0f));
    model =
        glm::rotate(model, rot_y * 3.14f / 5.f, glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    glDrawArrays(GL_TRIANGLES, 0,
                 numTris);  //(Primitives, Which VBO, Number of vertices)
