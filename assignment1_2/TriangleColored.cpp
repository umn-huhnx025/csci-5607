...
void changeColor(GLfloat* vertices) {
  std::swap(vertices[2], vertices[7]);
  std::swap(vertices[3], vertices[8]);
  std::swap(vertices[4], vertices[9]);
  std::swap(vertices[7], vertices[12]);
  std::swap(vertices[8], vertices[13]);
  std::swap(vertices[9], vertices[14]);
}
...
int main(int argc, char* argv[]) {
  ...
  SDL_Window* window = SDL_CreateWindow(
      "My OpenGL Program", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
  ...
  // SDL_Window* window = SDL_CreateWindow("My OpenGL Program", 0, 0, 800, 600,
  // SDL_WINDOW_FULLSCREEN|SDL_WINDOW_OPENGL);  SDL_Window* window =
  // SDL_CreateWindow("My OpenGL Program", 100, 100, 800, 600,
  // SDL_WINDOW_RESIZABLE|SDL_WINDOW_OPENGL);  SDL_Window* window =
  // SDL_CreateWindow("My OpenGL
  ...
  // 2 - The shader programs: the actual code telling the GPU how to draw the
  // model data  3 - A vertex buffer object (VBO): this is space to store the
  // model's data on the GPU  4 - A vertex array objet (VAO): this stores a
  // mapping between the shader inputs/attributes and the VBO data

  // Load the model data
  GLfloat vertices[] = {
      // We should read this from a files... for now lets hardcode it
      0.0f,  0.5f,  1.0f,
      0.0f,  0.0f,  // Vertex 1: postion = (0,.5) color = Red
      0.5f,  -0.5f, 0.0f,
      1.0f,  0.0f,  // Vertex 2: postion = (0,-.5) color = Green
      -0.5f, -0.5f, 0.0f,
      0.0f,  1.0f  // Vertex 3: postion = (-.5,-.5) color = Blue
  };
  ...
  glUseProgram(shaderProgram);  // Set the active shader program (only one can
                                // be used at a time)
